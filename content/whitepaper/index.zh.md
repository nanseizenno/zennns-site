...
它通过 PCN 前置控制节点，在系统进入目标状态、目标执行路径或目标物理执行阶段之前，汇总多源状态信号，将其映射为 C 条件、A 许可、E 执行链三类状态变量，并从 S 结构、D 动态、B 边界三个性质进行判定，形成 CAE-SDB 判定结果，再经过控制优先级仲裁，输出可执行、可记录、可复核的多路径控制结果。
...
PCN 设置在目标状态进入前，接收多源状态信号，完成 C / A / E 状态映射、S / D / B 判定、CAE-SDB 判定结果生成和控制优先级仲裁，再输出对应的多路径控制结果。
...
| PCN 设置位置 | 前置控制节点部署在目标状态进入前的哪个位置 |
| 触发时机 | 什么时候启动前置判定 |
| 多源状态信号 | 信号来自哪些设备、系统、接口或人工确认 |
| C 条件状态 | 哪些信号用于判断进入条件是否成立 |
| A 许可状态 | 哪些信号用于判断系统是否允许进入 |
| E 执行链状态 | 哪些信号用于判断进入后是否能够接续 |
| S 结构判定 | 信号、接口、映射、路径、许可来源是否定义、接入并可观测 |
| D 动态判定 | 当前状态是否有效、同步、未超时、可信 |
| B 控制边界判定 | 与 C / A / E 相关的时间窗口、重试上限、置信度、偏差范围等边界条件是否被触发 |
| CAE-SDB 判定结果 | 汇总 C / A / E × S / D / B 形成的结构化判定结果 |
| 控制优先级仲裁 | 对同时成立的多个判定结果进行控制优先级仲裁 |
| 多路径控制输出 | 根据仲裁结果输出最终控制路径 |
| 记录内容 | 记录输入、映射、判定、仲裁、控制输出、时间和追溯 ID |
...
B 判定检查与 C / A / E 状态相关的时间窗口、置信度、位置偏差、重试上限等预设控制边界是否被触发。

PCN 结合完整 CAE-SDB 判定结果进行控制优先级仲裁，再生成最终多路径控制输出。
...
结构缺失不能被当作对应状态已经成立。对于关键结构项，S 判定失败时不得输出 Allow；具体处置路径由完整 CAE-SDB Result 经控制优先级仲裁后确定。
...
D 判定用于确认当前状态是否仍可作为本次状态迁移的有效依据。对于关键动态状态，若其已超时、未刷新、冲突或不同步，则不能继续作为 Allow 的有效依据；具体控制路径由完整 CAE-SDB Result 经控制优先级仲裁后确定。
...
B 判定与 S、D 判定共同形成 CAE-SDB Result，最终控制输出还需要经过 Arbitration 后生成。
...
|---|---|---|---|
| S 结构完整性 | 条件信号是否定义、接入、可观测 | 许可来源是否定义、接入、可观测 | 执行链、下游、回退、异常路径是否定义、接入、可观测 |
| D 动态时序有效性 | 条件信号是否超时、抖动、冲突、未刷新 | 许可是否延迟、撤销、冲突、未同步 | 执行链状态是否未刷新、阻塞、反馈超时或处于切换中 |
| B 控制边界 | 时间窗口、重试次数、识别置信度、位置或姿态偏差是否触发设定边界 | 许可等待时限、授权范围或其他许可相关边界条件是否触发 | 下游等待时间、容量、备用路径可用范围或其他执行链边界条件是否触发 |


![CAE-SDB 正交判定结构](/images/tpca/03-cae-sdb-matrix.png)
图4：C / A / E 表示状态变量域，S / D / B 表示判定性质；矩阵结果形成 CAE-SDB Result，并作为后续控制优先级仲裁和多路径控制生成的输入。

现场信号先映射为 C / A / E，再接受 S / D / B 分析。

S 检查判定结构是否存在。

D 检查当前运行状态是否仍然有效。

B 判断相关状态是否进入预先定义的控制边界。

完成 CAE-SDB 判定以后，结果进入 Arbitration，PCN 再生成最终 Multipath Control。
...
PCN 输出控制路径时，不应简单按异常数量决定，也不能把某一个 CAE-SDB 判定结果直接对应为固定控制动作。

关键 A 可以构成独立必要约束。  
关键许可不成立时，不得输出“允许进入目标状态”。

关键结构缺失时，不应假定对应状态已经成立。

关键动态状态失效时，不应继续使用已经超时、未刷新、冲突或不同步的状态作为放行依据。

B 用于判断相关 C / A / E 状态是否触发预设控制边界，本身不直接决定最终控制路径。

当多个 CAE-SDB 判定结果同时存在时，需要经过控制优先级仲裁，再生成最终多路径控制输出。

只有必要 C 条件成立、关键 A 许可成立、必要 E 执行链可接续，关键 S / D 判定有效，且未触发禁止进入目标状态的 B 边界，并且仲裁结果不存在更高优先级的限制性结果时，才允许进入目标状态。

本文只说明控制优先级的基本原则，不展开完整仲裁规则和 PCN Runtime 实现细节。
...
| C / A / E 映射结果 | 记录状态变量域映射结果 |
| S / D / B 判定结果 | 记录结构、动态和控制边界判定结果 |
| CAE-SDB 判定结果 | 记录各状态变量域与判定性质组合形成的结构化结果 |
| Arbitration 结果 | 记录多个判定结果之间的控制优先级仲裁结果 |
| Multipath Control | 记录最终输出的控制路径 |
| 时间戳 / 追溯编号 | 支持 HMI 显示、事件复盘、改善对比和后续分析 |

记录的价值不在于堆积更多日志，而在于形成可复盘的状态迁移判定履历。
...
> 注：  
> AI 辅助分析应以 PCN Trace 为输入基础。PCN 已经将现场信息整理为当前状态、目标状态、多源信号、C / A / E 映射、S / D / B 判定、CAE-SDB Result、Arbitration Result、Multipath Control 和追溯记录。  
> 相比直接读取原始报警、设备日志和人工备注，这类结构化履历更能提供工程上下文。AI 的作用应限定在履历比较、模式归纳、候选改善方向整理和报告生成，不直接控制现场。
...
TPCA / CAE-SDB 在单元进入目标物理执行阶段前，把多源状态信号整理为 C / A / E，并进行 S / D / B 判定，形成 CAE-SDB Result，再经过控制仲裁输出多路径控制结果。
...
在群控协同应用中，还可以结合群体状态指标进行停滞识别和结构分型。
...
- 把控制边界判定、仲裁结果以及降级、禁止、安全锁定、人工确认等最终控制输出形成记录；
...
在这些场景中，请求已经生成，并不代表目标执行路径已经成立。

仍然需要判断：

- 请求条件是否满足；
- 权限、预算、租户授权、工具调用许可是否成立；
- 模型实例、工具链、接口服务、队列资源是否可执行；
- 请求最终应进入高成本路径、澄清路径、降级路径、等待路径，还是阻断路径。

这一方向属于扩展应用。

制造系统中的核心主线仍然是自动化执行单元前置判定和 MES / WCS 协同停滞诊断。
...
PCN 设置在这些目标状态入口之前，把多源信号整理为 C / A / E 状态变量，通过 S / D / B 判定识别结构完整性、动态时序有效性和控制边界状态，形成 CAE-SDB Result，再经过控制优先级仲裁输出最终控制路径。

它不替代 PLC、MES、WCS 或安全系统。
...
- 当前状态是否仍然有效；
- 是否触发控制边界；
- 多个判定结果同时出现时，哪个结果具有更高控制优先级；
- 最终应该输出哪条控制路径。

一个 PCN 能被定义、配置、复用和复盘，多个 PCN 就能进一步形成状态迁移前置控制网络。
...
说明 TPCA / CAE-SDB 与状态机、安全互锁、报警诊断、AI 诊断、MES / WCS 调度和形式化验证之间的边界。

### [TPCA / PCN 建立在什么工程基础上？——五个基础工程共识](/zh/notes/engineering-foundations-of-tpca-pcn/)

从状态迁移、许可约束、执行链接续、动态时序有效性和控制边界五个基础工程共识理解 TPCA / PCN 的工程基础。
...
通过十个具体工程问题检验是否真正理解状态迁移入口、PCN、CAE-SDB、控制仲裁和多路径控制之间的关系。
...
如果希望继续阅读工程基础、技术争议、理解测试、PCN 工程节点、PCN Network、OEE 与 PCN 等专题，可以阅读：
...
本文为 TPCA / CAE-SDB 状态迁移前置控制架构的公开白皮书。

Public Version 1.0 发布于 2026-07-01。  

Public Version 1.1 更新于 2026-08-18，主要更新 CAE-SDB Result、Arbitration、Multipath Control、PCN Trace 的层级表达，并补充相关技术札记链接。

作者：全野南政 / Nansei Zenno  

建议引用：
```text
全野南政 / Nansei Zenno，《TPCA / CAE-SDB 状态迁移前置控制架构》，公开白皮书，Public Version 1.1，2026-08-18，https://zennns.com/zh/whitepaper/
```
