...
> **システムが明確な Target State へ進もうとするとき、関連状態は各システムに存在していても、本 Target State Entry でなぜ移行、待機、阻止、分流となるのかが、統一されたエンジニアリング対象として整理されていない。**
...
- 次に待機、再認識、再サンプリング、再位置決め、戻り搬送 / リターン、異常品分岐、移行禁止、人による確認のどれを選択すべきか。
...
複数の既存機構が一つの Target State への移行を共同で決定する場合、TPCA / PCN は、同一の Target State Entry を中心として前判定、制御、履歴を構成する。
...
PCN は、明確な Target State Entry の前に配置し、本 Target State への移行に関係する入力、判定、Arbitration、制御、履歴を一つの単位として扱う。
...
ここでいう「移行前―入口―移行後」は、C / A / E が対応するエンジニアリング上の機能位置を示すものであり、E を Target State へ進入した後に判定するという意味ではない。C、A、E は、いずれも Target State Entry の前に PCN で前判定する。
...
安全停止、退避、リターン、再認識、再投入などの経路へ進む場合も、それぞれ新しい Target State Entry を中心として整理できる。
