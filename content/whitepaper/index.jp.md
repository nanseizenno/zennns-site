...
description: "TPCA 状態遷移前制御アーキテクチャと、そのエンジニアリングノードである PCN について説明する。明確な Target State Entry を中心として複数ソース状態信号を整理し、C / A / E 状態マッピングと S / D / B 判定を行って CAE-SDB Result を形成する。さらに、Arbitration を経て Multipath Control を出力し、PCN Trace に状態遷移判定履歴を記録する一連の構造を示す。"
date: 2026-07-01
lastmod: 2026-08-21
...
**PCN（Pre-Control Node）** は、具体的な Target State Entry の前に配置する前制御ノードであり、このエンジニアリング対象を実システム上で扱うためのノードである。

PCN は、明確な Target State Entry を中心として複数ソース状態信号を整理し、C / A / E 状態マッピングと S / D / B 判定を行って CAE-SDB Result を形成する。その後、Arbitration を経て Multipath Control を出力し、PCN Trace に本判定および実行の履歴を記録する。
...
- 問題が構造欠落、動的時系列有効性、制御境界のどれに該当するのか。
...
![複雑工程システムにおける共通状態遷移問題](/images/tpca/01-common-state-transition-problem.png)

図1：複数の現場問題は、システムが Target State へ進む直前に集中して発生することが多い。

TPCA は、状態遷移前制御の全体アーキテクチャを定義する。

PCN は、明確な Target State Entry の前に配置し、複数ソース状態信号を取得して C / A / E 状態マッピング、S / D / B 判定、CAE-SDB Result の形成を行う。その後、Arbitration を経て対応する Multipath Control を出力し、PCN Trace を記録する。
...
T は、本状態および判定とともに保持する時間情報であり、状態の前後関係、D：Dynamics の判定、PCN Trace の形成に使用する。
...
これらの仕組みは基本運転を支えるが、システムが次の状態へ進めない場合、結果が次のような単純な表現に集約されることがある。
...
これらは、条件を満たさない動作を停止させるためには有効である。一方、それだけでは次の内容を直接説明できない場合がある。

- 問題が対象条件、許可状態、Execution Chain のどこにあるのか。
- 現在状態が構造欠落、動的時系列有効性、制御境界のどれに該当するのか。
...
TPCA は、システムが Target State、目標実行経路、または目標物理実行段階へ進む前の前判定と Multipath Control を対象とし、明確な Target State Entry を前制御の基本エンジニアリング対象として扱う。

Target State には、例えば次のものがある。

- 自動化実行ユニットの次の物理実行段階
- ロボットユニットのピックアップ、配置、回流、異常分流段階
- 圧入、検査、搬送引渡しなどの工程段階
- AGV / AMR 群制御システムにおける継続可能な協調実行状態
- MES / WCS における目標実行経路
- 製造 DX における人員、設備、システム、プロセス間の重要な状態切替
- デジタル呼出しシステムにおける目標実行経路

TPCA が確認する主な事項は、次のとおりである。

- Target State へ進むための条件が成立しているか。
- 必要な許可が成立しているか。
- Target State へ進んだ後、Execution Chain を継続できるか。
- 現在の判定構造が完全であるか。
- 現在状態が現在も有効であるか。
- 制御境界に到達しているか。
- 複数の判定結果から、どのように最終制御経路を決定するか。
- 本状態遷移をどのように追跡可能な履歴として残すか。

TPCA は、既存の PLC、ステートマシン、SFC、Interlock、安全システム、MES、WCS を置き換えることを目的としない。

TPCA の役割は、

> **複数の既存機構が一つの Target State への進入を共同で決定する場合に、同一の Target State Entry を中心として、明確な前判定、制御、履歴を構成すること**
...
PCN は、明確な Target State Entry の前に配置し、本 Target State への進入に必要な入力、判定、Arbitration、制御、履歴を一つの単位として扱う。
...
と遷移した後、C の状態内容が A と同一であっても、システムが過去の A に戻ったことにはならない。
...
新しい Target State が過去の状態と同一または類似した内容を持っていても、実際には新しい状態インスタンスである。
...
製造 DX では、人員、設備、システム、プロセス間の重要な状態切替入口に PCN を配置できる。
...
| 分析項目 | 内容 |
|---|---|
| Current State | システムが現在どの状態にあるか |
| Target State | システムが次にどの状態へ進もうとしているか |
...
| C：Condition | Target State へ進むための事実条件 |
| A：Authority | Target State への進入を許可する状態 |
| E：Execution Chain | Target State へ進んだ後に Execution Chain を継続できるか |
| S：Structure | 必要な信号、インターフェース、マッピング、許可元、Execution Chain 境界が完全であるか |
| D：Dynamics | 現在状態が有効、同期済み、未タイムアウトであり、判定根拠として使用可能か |
| B：Boundary | 現在状態が事前定義された制御境界に到達しているか |
...
PCN に取り込む状態は、本 Target State Entry に直接関係し、C / A / E 状態マッピング、S / D / B 判定、または最終制御経路の決定に使用するものとする。
...
ロボットが待機段階からピックアップ段階へ進む場合を例とする。

Current State：ロボット待機。メインコンベヤがワークを搬送し、画像認識システムが認識を完了している。

Target State：ロボットがピックアップ段階へ進む。

PCN 配置位置：ピックアップ動作を開始する直前。
...
ここでは、「戻り経路」「回流経路」などの現場エンジニアリング用語をそのまま使用する。

これらは、経路の用途を示す名称である。

システムが実際にこれらの経路へ進む場合、TPCA では次の状態遷移として扱う。

```text
Current State → New Target State / Target Path
```

その後、関連状態に対して S / D / B 判定を行い、CAE-SDB Result を形成する。さらに Arbitration を経て、最終的な Multipath Control を出力する。
...
MES / WCS 協調システムでも、タスク実行、エリア進入、ステーション受入、リソース解放、下流処理、結果書戻しなどの Target State Entry に複数の PCN を配置できる。

PCN Network は、Target State Entry と、それらのエンジニアリング上の関係を表す。

状態タイプの関係として循環構造を持つ場合があるが、実運用上の状態遷移は時間方向へ継続して進む。

したがって、

> **PCN Network は状態タイプとして循環関係を持つことができるが、実際の状態インスタンスが過去へ戻ることはない。**

PCN Network により、例えば次の内容を確認できる。

- どの Target State Entry で問題が頻発しているか。
- どの許可が継続的な阻止要因となっているか。
...
PCN は、既存の PLC、SFC、Interlock、安全システム、MES、WCS と役割を分担するものであり、これらの成熟した機構を置き換えるものではない。
...
C：Condition は、Target State へ進む前に、本 Target State Entry に関係する事実条件が成立しているかを示す状態変数領域である。
...
- 目標ステーションが定義されているか。
- バッファが使用可能であるか。
- オーダー、工程、現場条件が要求を満たしているか。

## 4.2 A：Authority

A：Authority は、システムとして Target State への進入が許可されているかを示す状態変数領域である。
...
重要な許可が成立していない場合、C：Condition と E：Execution Chain が成立していても、Target State へ進入してはならない。

## 4.3 E：Execution Chain

E：Execution Chain は、Target State へ進んだ後、後続の実行チェーンを継続できるかを示す状態変数領域である。

E：Execution Chain は、単体設備の Ready と同一ではない。
...
「戻り」「回流」などの用語は、E：Execution Chain における後続実行経路の用途を示す。

これらの経路へ実際に進む場合、TPCA では新しい Target State または目標実行経路への状態遷移として扱う。

E：Execution Chain が確認するのは、

> **Target State へ進んだ後、後続の実行チェーンを継続できるか。**
...
B：Boundary は、C / A / E に関係する状態が、事前に定義された制御境界に到達しているかを判定する。

制御境界の判定に使用するエンジニアリングパラメータには、例えば次のものがある。
...
- 容量境界
- リソース占有境界
- その他、Target State Entry に関係する境界パラメータ

B：Boundary の具体的な判定境界は、Target State、リスクレベル、設備特性、工程条件、制御方針に基づいて設定する。

B：Boundary 自体は制御経路ではない。

制御境界へ到達した後に、待機、再サンプリング、回流、人による確認、進入禁止、その他の制御経路のどれを選択するかは、完全な判定結果に対する Arbitration によって決定する。
...
| T2 | C(T2) | A(T2) | E(T2) | 安定、取消、競合、失効などへ変化する可能性あり |
| Tn | C(Tn) | A(Tn) | E(Tn) | Target State へ進む直前の判定根拠 |

現場では、例えば次のような変化が発生する。

- 画像認識結果は T0 では有効であったが、その後失効する。
- 安全許可は T0 では成立していたが、T1 で取り消される。
...
| 判定性質 / 状態変数領域 | C：Condition | A：Authority | E：Execution Chain |
|---|---|---|---|
| S：Structure | Condition に必要な信号、インターフェース、マッピングが完全であるか | Authority の取得元、インターフェース、関係が完全であるか | 下流、戻り、退避、異常経路、書戻し経路が完全であるか |
...
| B：Boundary | Condition が事前定義された制御境界に到達しているか | Authority が事前定義された制御境界に到達しているか | Execution Chain が事前定義された制御境界に到達しているか |

![CAE-SDB 直交判定構造](/images/tpca/03-cae-sdb-matrix.png)

図4：C / A / E は状態変数領域、S / D / B は判定性質を示し、それらの組合せによって CAE-SDB Result を形成する。

CAE-SDB Result は、最終制御動作ではない。
...
> **問題がどの状態変数領域にあり、どの判定性質に該当するか。**

を示す。

Arbitration は、

> **複数の判定結果、重要な許可、制御制約が同時に存在する場合に、どの制御結論を採用するか。**
...
これらの用語は、現場で使用されるエンジニアリング上の意味をそのまま維持する。

TPCA の状態遷移視点では、Multipath Control の共通的な役割は、

> **現在の判定結果に基づいて、システムが次に進む Target State または目標実行経路を決定すること**

である。

したがって、「回流」「戻り」などは経路用途を示す名称であり、状態インスタンスが過去へ戻ることを意味しない。
...
| 判定結果 | 内容 |
|---|---|
| C-S | Condition 領域に Structure の問題がある |
| C-D | Condition 領域に Dynamics の問題がある |
| C-B | Condition 領域が Boundary に到達している |
| A-S | Authority 領域に Structure の問題がある |
| A-D | Authority 領域に Dynamics の問題がある |
| A-B | Authority 領域が Boundary に到達している |
| E-S | Execution Chain 領域に Structure の問題がある |
| E-D | Execution Chain 領域に Dynamics の問題がある |
| E-B | Execution Chain 領域が Boundary に到達している |

同一の Target State Entry に対する判定で、複数の CAE-SDB Result が同時に存在する場合がある。

## 5.3 Arbitration の基本原則

本公開ホワイトペーパーでは、Arbitration の基本原則のみを示す。

重要な A：Authority は、独立した必要条件となり得る。

重要な許可が成立していない場合、Target State への進入を許可しない。

重要な Structure が欠落している場合、関連状態が成立しているものとして扱ってはならない。

重要な Dynamics が失効している場合、タイムアウト、未更新、競合、非同期などの状態を、そのまま進入許可の根拠として使用してはならない。

複数の CAE-SDB Result が同時に存在する場合、Arbitration を経て最終的な Multipath Control を形成する。

必要な Condition、重要な Authority、Execution Chain が成立し、かつそれらより高い優先度を持つ制限制約が存在しない場合に限り、Target State への進入を許可する。

元の Target State へ進めない場合、Arbitration は完全な判定結果に基づいて、待機、回流、再認識、人による確認、安全状態、その他の Target State または目標実行経路を選択できる。

完全な Arbitration ルール、内部実行ライフサイクル、設定構造、具体的な制御パラメータは、本公開ホワイトペーパーの範囲には含めない。
...
| 記録項目 | 内容 |
|---|---|
| Current State | システムが当時どの状態にあったか |
| Target State | システムがどの状態へ進もうとしていたか |
| 主要入力状態 | 本状態遷移に直接関係する状態 |
| 時間情報 T | 本状態および判定の時間位置 |
| バージョン / 対象情報 | 状態バージョンおよび対象との関連 |
| C / A / E 状態マッピング | 状態変数領域 |
| S / D / B 判定 | 判定性質 |
...
- どの Boundary へ高頻度で到達しているか。
- どの制御経路が高頻度で選択されているか。
- 同一の状態タイプが繰り返し現れていても、実際の状態インスタンスがどのような異なる遷移履歴を持っているか。
- エンジニアリング変更後に、状態遷移の傾向がどのように変化したか。

したがって、TPCA / PCN は現在の状態遷移制御だけでなく、後続の設計改善、設備再利用、エンジニア教育、プロジェクト引継ぎ、製造 DX のための構造化履歴としても使用できる。

AI は、PCN Trace に基づく履歴比較、パターン分析、改善候補の整理、レポート生成を補助できる。

AI 分析は、PCN または既存の現場制御ロジックに代わって確定的制御を実行するものではない。
...
図5：TPCA / PCN は、自動化実行ユニット、群制御協調、製造 DX、デジタル呼出しガバナンスなど、異なるエンジニアリング対象へ展開できる。
...
PCN は、目標物理実行段階へ進む前に配置し、複数ソース状態に対して C / A / E 状態マッピングと S / D / B 判定を行って CAE-SDB Result を形成する。その後、Arbitration を経て Multipath Control を出力する。

異常発生後に、安全停止、退避、回流、再認識、再投入などの経路へ進む場合も、それぞれ新しい Target State Entry として扱う。
...
群制御・協調層の PCN は、複数ソース状態を取得し、C / A / E 状態マッピング、S / D / B 判定を行う。さらに、群全体の指標を用いて停滞識別と構造分析を行う。
...
製造 DX は、データ収集、可視化、レポート、ダッシュボードの構築から開始する場合が多い。
...
ここで使用する「復帰」「再投入」などの現場用語は、そのまま維持する。
...
リクエストが生成されたことと、目標実行経路が成立していることは同じではない。

さらに、次の内容を判定する必要がある。

- リクエスト条件が成立しているか。
- 権限、予算、テナント認可、ツール呼出し許可などが成立しているか。
- モデル、ツールチェーン、インターフェースサービス、キューリソースが実行可能であるか。
- 高コスト経路、確認経路、縮退経路、待機経路、阻止経路のどれへ進むか。

この領域は、TPCA / PCN の拡張適用である。

製造システムにおける主要な適用対象は、自動化実行ユニット前制御、MES / WCS 協調停滞、製造 DX における複数システム横断の状態遷移である。
...
PCN は、明確な Target State Entry の前に配置し、本状態遷移に関係する複数ソース状態を C / A / E 状態変数領域へマッピングし、S / D / B 判定を行って CAE-SDB Result を形成する。その後、Arbitration を経て最終的な Multipath Control を出力し、PCN Trace に本状態遷移の履歴を記録する。
...
TPCA / PCN は、PLC、SFC、Interlock、安全システム、MES、WCS を置き換えるものではない。

TPCA / PCN が補完するのは、

> **明確な Target State Entry を、独立して設計・判定・制御・記録可能なエンジニアリング対象として扱うこと**
...
これにより、通常実行、回流、戻り、復帰、縮退、異常処理、再投入などの現場用語を維持したまま、それぞれを次の状態遷移として統一的に扱うことができる。
...
```text
全野南政 / Nansei Zenno，《TPCA / PCN 状態遷移前制御アーキテクチャ》，公開ホワイトペーパー，Public Version 1.3，2026-08-21，https://zennns.com/zh/whitepaper/
```
