...
复杂制造系统的问题，不只是设备多、信号多、日志多，而是状态迁移入口越来越多。

TPCA / CAE-SDB 关注的正是这些入口。  
PCN 设置在目标状态进入前，把多源信号整理为 C / A / E 状态变量，再通过 S / D / B 判定输出控制路径和记录结果。

它不替代 PLC、MES、WCS 或安全系统。  
它补足的是进入前的工程判定层：哪些条件成立，哪些许可成立，执行链能不能接续，下一步应走哪条路径。

当一个 PCN 能被定义、复用和复盘，多个 PCN 就能形成状态迁移前置控制网络。  
...
以下技术札记用于补充理解 TPCA / CAE-SDB 与既有方法、PCN 工程节点、PCN 网络和状态迁移条件设计之间的关系。

### [TPCA / CAE-SDB 与既有工业自动化理论的关系](/zh/notes/tpca-existing-theories/)

用于理解 TPCA / CAE-SDB 与状态机、安全互锁、报警诊断、AI 诊断、MES / WCS 调度和形式化验证之间的边界关系。

### [为什么 PCN 是 TPCA 的最小工程单元？](/zh/notes/why-pcn-is-minimal-engineering-unit/)

用于理解 TPCA 如何通过 PCN 落到具体状态入口、输入、判定、输出和记录。

### [多个 PCN 如何形成状态迁移前置控制网络？](/zh/notes/pcn-network-state-transition-pre-control/)

用于理解多个 PCN 如何按目标状态入口连接成状态迁移前置控制网络，并形成“这里需要增加一个 PCN 点”的工程讨论语言。

### [为什么 OEE 之后还需要 PCN？](/zh/notes/why-pcn-after-oee/)

用于理解 OEE 与 PCN 之间的互补关系，以及为什么设备数据之外还需要状态迁移前置判定。

### [为什么状态迁移条件必须显式化？](/zh/notes/why-state-transition-conditions-must-be-explicit/)
...
本文为 TPCA / CAE-SDB 状态迁移前置控制架构的公开白皮书版本。

Public Version 1.0 主要用于说明以下内容：

- TPCA / CAE-SDB 的工程定位；
- PCN 前置控制节点的基本作用；
- C / A / E 状态映射与 S / D / B 判定的基本关系；
- 多路径控制输出的公开层级表达；
- 自动化执行单元、MES / WCS 协同停滞、生产 DX 和数字调用治理等典型应用方向。
