...
TPCA / CAE-SDB 是一套面向目标状态进入前的工程判定结构。
...
制造现场已经具备 ready、interlock、handshake、报警、状态码、MES 记录和 WCS 日志。它们能够支撑基础运行，也能记录大量状态。
...
{{< diagram
src="/images/tpca/01-common-state-transition-problem.png"
alt="复杂工程系统中的共通状态迁移问题"
caption="图1：多个现场现象往往集中发生在进入目标状态之前。"
>}}

TPCA / CAE-SDB 用 PCN 前置控制节点补足这一层结构。

PCN 在目标状态进入前完成 C / A / E 状态映射和 S / D / B 判定，并输出允许进入、等待、重识别、回流、异常分流、下游协调、资源释放、降级执行、禁止进入、安全锁定、人工确认、增强记录等控制路径。

本文定位为公开白皮书，用于说明 TPCA / CAE-SDB 的工程定位、基本结构和典型应用方向。
...
这些机制能支撑基础运行，但在系统不能进入下一状态时，输出经常被压缩成少数结果：OK / NG、ready / not ready、true / false、允许 / 不允许、报警 / 正常、Waiting / Pending / Blocked。

这些结果能阻止错误动作，却往往不足以支撑快速排查、稳定复用和持续改善。现场真正需要进一步知道：这个 NG 来自对象条件、许可状态，还是执行链承接；这个 Waiting 是信号未接入、信号过期、许可未刷新，还是下游不能承接；这个 Ready 是否只代表本机 ready；这个任务是否具备进入目标执行路径的条件；这个报警是源头问题，还是后续结果；下一步应等待、重识别、回流、分流、协调、降级、禁止，还是人工确认。
...
很多状态迁移判断长期存在于工程师经验、梯形图、调试记录、HMI 画面、设备接口和现场习惯中。TPCA / CAE-SDB 的目标，是把这些隐含判断整理成可描述、可检查、可记录、可复用、可交接的工程结构。
...
PCN 部署在目标状态进入前，负责接收多源状态信号，并完成以下处理：
...
{{< diagram
src="/images/tpca/04-pcn-node-position.png"
alt="PCN 前置控制节点的位置与工作方式"
caption="图2：PCN 位于当前状态与目标状态之间，在进入前完成信号整理、结构判定和控制路径输出。"
>}}

![PCN 前置控制节点的位置与工作方式](/images/tpca/04-pcn-node-position.png)
图2：PCN 位于当前状态与目标状态之间，在进入前完成信号整理、结构判定和控制路径输出。
...
{{< diagram
src="/images/tpca/02-tpca-process-chain.png"
alt="TPCA 的基本处理链条"
caption="图3：TPCA 从当前状态、目标状态和多源信号出发，形成判定结果、控制路径和追溯记录。"
>}}

这一链条的核心意义在于：

false、NG、not ready、pending 不再作为终点。
...
第二阶段是结构化显示。  
把 PCN 判定结果转化为画面、事件报告、结构化诊断码和复盘记录。
...
无论采用哪种部署方式，PCN 都应在目标状态进入前进行判定，不绕过既有安全机制和现场控制规则。

## 第 3 章 C / A / E 状态映射：条件、许可与执行链

### 3.1 C = Condition，条件状态
...
PCN 不只判断“当前能不能进”，还要判断这个进入判断是否完整、可信、处于可控边界。

## 第 4 章 S / D / B 判定：结构、动态与边界

### 4.1 S = Structure，结构完整性
...
B 类问题把“灰色状态”转化为明确路径：继续等待、回流、降级、禁止进入、人工确认或安全锁定。

### 4.4 CAE-SDB 的组合关系

C / A / E 表示问题来自哪个状态变量域。S / D / B 表示这个变量域的问题属于什么性质。两者组合后，形成 CAE-SDB 判定结构。
...
{{< diagram
src="/images/tpca/03-cae-sdb-matrix.png"
alt="CAE-SDB 正交判定结构"
caption="图4：C / A / E 表示问题来自哪里，S / D / B 表示问题属于什么性质，矩阵结果用于决定控制路径。"
>}}

CAE-SDB 不是静态分类表。
现场信号先映射为 C / A / E，再在 t0～tn 的状态迁移过程中接受 S / D / B 分析，最后的输出不是“九宫格标签”，而是控制路径。

## 第 5 章 从判定结果到多路径控制和状态记录
...
关键安全许可优先。关键 A 许可不成立时，优先禁止进入或安全锁定。

结构缺失不应被当作状态成立。动态异常需要等待稳定、重采样或重新同步。边界状态需要根据风险等级选择等待、降级或人工确认。C、A、E 均满足且动态可信时，才允许进入目标状态。

本文只说明控制优先级的基本原则，不展开完整仲裁规则和现场实现细节。

### 5.4 状态记录与追溯

PCN 判定结果应形成可复核记录。建议至少记录以下内容。
...
TPCA / CAE-SDB 不只用于当前排查，也可以支撑后续设计改善、设备复用、工程培训、项目交接和生产 DX。

## 第 6 章 制造系统中的应用方向

{{< diagram
src="/images/tpca/05-tpca-application-map.png"
alt="TPCA / CAE-SDB 的应用方向"
caption="图5：TPCA / CAE-SDB 作为母框架，可展开到群控协同层、物理执行单元层和数字调用治理层。"
>}}
...
在自动化执行单元中，这套结构用于判断单元为什么不能进入下一阶段。在 MES / WCS 协同系统中，它用于判断群体为什么停。在生产 DX 中，它用于把人员、设备、系统和流程之间的状态迁移条件显式化，并沉淀为后续设计、复盘、培训和改善可以继续使用的工程资产。

TPCA / CAE-SDB 的重点不在于增加更多报警，也不在于替换既有 PLC、MES、WCS 或安全系统。它补足的是目标状态进入前的判定层：进入前看什么信号，信号属于哪个变量域，问题属于什么性质，下一步走哪条路径，以及这次判断如何被记录和复用。
