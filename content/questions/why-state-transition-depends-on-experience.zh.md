...
设备能运行，程序也能维护，HMI 也能显示状态。  
但一旦现场出现“Ready 了不动”“Waiting 查不清”“报警很多但不知道先看哪个”“MES / WCS 有记录但解释不了停滞”这类问题，最后还是要找熟悉项目的人。
...
- 有的项目会检查信号时间戳，有的项目只检查 OK / NG。
- 有的项目会区分安全许可、上位许可和下游许可，有的项目全部压缩成 interlock。
...
相反，很多现场系统能稳定运行，正是因为工程师靠经验把大量隐含条件补上了。

真正的问题在于：这些经验没有被整理成统一的状态迁移设计结构。

## 个人经验为什么会变成关键因素

状态迁移设计涉及的内容很多。

一个动作能不能进入下一阶段，通常要同时考虑：
...
结果就是，系统表面上有程序、有报警、有状态、有日志，但真正理解“为什么能进入下一状态、为什么不能进入下一状态”的人，仍然是熟悉整个项目的人。

这就是状态迁移设计长期依赖个人经验的原因。

## 传统控制逻辑为什么难以沉淀这类经验

传统控制逻辑通常围绕动作、步骤、互锁、报警和设备状态展开。
...
当项目规模较小时，经验丰富的工程师可以靠记忆和理解把这些内容串起来。
...
设备接口越来越多；  
异常路径越来越多；  
上位系统与现场设备之间的状态关系越来越多；  
新人理解成本越来越高；  
不同项目之间复用困难。

因此，状态迁移设计长期依赖个人经验，并不是因为现场没有控制逻辑，而是因为控制逻辑缺少统一的状态迁移表达方式。
...
同类设备、同类单元、同类产线在新项目中重复设计。  
经验可以迁移，但缺少标准化结构时，复用效率很低。
...
A 类状态中，关键许可应作为独立必要约束。  
只要关键许可未成立，即使条件和执行链看起来满足，也不应进入目标状态。
...
很多老工程师能快速判断问题，就是因为他在脑中已经隐含完成了这些区分。

把这些区分写出来，状态迁移设计才有可能从个人经验变成可复用结构。
...
- 每次判定都有记录；
- 每次异常处理都能复盘；
- 新项目可以继承旧项目的判定结构，而不是只复制程序片段。

这样，工程师的经验不再只存在于个人脑中，而是沉淀为模板、矩阵、诊断画面、履历记录和项目标准。
...
传统项目中，这些内容分散在 PLC 程序、机器人程序、HMI 画面、MES / WCS 状态、接口文档、调试记录和工程师经验中。

系统可以运行，但状态迁移条件是否完整、异常边界是否清楚、执行链是否接得住，仍然依赖熟悉项目的人判断。
