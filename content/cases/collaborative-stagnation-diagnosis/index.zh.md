...
| 判定性质 | 典型确认内容 |
|---|---|
| S：Structure | MES 与 WCS 的任务映射、资源锁来源、站点承接接口、路径资源关系、许可来源、结果回写接口是否已经定义、接入并可观测 |
| D：Dynamics | 工单与任务是否同步，主体位置是否及时更新，站点状态是否仍有效，许可是否被撤销，路径占用是否冲突，执行反馈是否超时 |
| B：Boundary | 等待时间、资源占用时间、站点容量、许可时间窗、资源使用范围、重试次数等是否处于预先定义的允许边界内 |
