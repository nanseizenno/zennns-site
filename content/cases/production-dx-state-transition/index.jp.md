...
C / A / E は状態変数領域、S / D / B はそれらの状態を確認するための判定性質である。

本事例では、この関係を次の CAE-SDB Matrix として整理できる。

| 判定性質 | C：Condition（条件状態） | A：Authority（許可状態） | E：Execution Chain（実行チェーン状態） |
|---|---|---|---|
| S：Structure（構造完全性） | 部品 ID と作業指示、ロットと品質情報などの対応関係が定義・接続され、観測可能か | 資格マスターや品質承認など、許可に必要な情報源と対象との関係が定義・接続され、観測可能か | セルと品種、治具・工具、実績書戻し経路などの関係が定義・接続され、観測可能か |
| D：Dynamics（動的時系列有効性） | 品質情報が現在ロットに対応しているか、作業指示が最新版か | 作業者ログインが現在有効か、作業者資格が失効していないか | セル状態が更新されているか |
| B：Boundary（制御境界） | 品質測定値、部品位置、セル前仕掛品数などが事前に定義された許容範囲内にあるか | — | バッファ容量、締付工具設定、待機時間などが事前に定義された許容範囲内にあるか |

各状態は、C / A / E のいずれかの状態変数領域へ整理したうえで、必要な S / D / B 判定を行う。CAE-SDB Matrix は、状態変数領域と判定性質の対応関係を整理するために用いる。
