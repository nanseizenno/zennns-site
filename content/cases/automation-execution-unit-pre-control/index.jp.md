...
| 判定特性 | 代表的な確認内容 |
|---|---|
| S：Structure（構造完全性） | 今回のピックアップ入口に必要な状態について、信号、インターフェース、マッピング、許可元、実行チェーン上の接続関係、および対象との対応関係が定義・接続され、観測可能であるか |
| D：Dynamics（動的時系列有効性） | 現在の状態を今回のピックアップ入口の判定根拠として引き続き使用できるか。画像認識結果の更新、ワーク追跡の同期、許可の撤回、ロボット・下流状態の遅延や未更新、状態切替中などを確認する |
| B：Boundary（制御境界） | 現在有効な状態の値または範囲が、認識信頼度、ワーク位置、姿勢、下流容量、時間窓など、事前に定義した許容範囲、しきい値、偏差または制御境界内にあるか |
...
本事例では、「ピックアップ段階への移行」という Target State Entry に対して、代表的なエンジニアリング上の問題を次のように整理できる。

| 判定座標 | 本事例における代表的なエンジニアリング上の問題 |
|---|---|
| C-S | 画像認識インターフェースが未接続である。認識結果の項目や対象との対応関係が未定義である。画像座標系とロボット座標系のマッピングが未設定である。ワークと認識結果の追跡関係が確立されていない |
| C-D | 画像認識結果が更新されていない。認識結果が現在の有効時間を超過している。ワーク追跡とコンベヤ上の位置が同期していない。認識結果が現在とは異なるワークに対応している |
| C-B | 現在有効な認識信頼度が許容しきい値を下回っている。ワーク位置がピックアップ可能範囲外にある。姿勢偏差が事前に定義した許容範囲を超えている |
| A-S | 安全許可、エリア許可、PLC 放行、上位システム許可などの許可元が未定義である。許可インターフェースやマッピングが未設定である。必要な許可状態を現在の PCN から取得できない |
| A-D | 許可状態が更新されていない。判定中に許可が撤回された。許可情報に遅延がある。安全状態またはエリア状態が切替中であり、現在の判定根拠として使用できない |
| A-B | 現在有効な許可が適用可能範囲外である。許可の有効時間窓が今回の入口条件を満たしていない。エリアまたは資源許可が事前に定義した使用境界を超えている |
| E-S | 現在のピックアップ段階に必要な Robot Ready、ロボット経路、グリッパ / 真空状態、正常品置場の受入状態、結果書戻し経路などのインターフェースまたは接続関係が未定義・未接続である |
| E-D | 現在のピックアップ段階に必要な Robot Ready やロボット経路状態が更新されていない。グリッパ / 真空状態と現在ワークの対応が同期していない。正常品置場の受入状態に遅延や失効がある。結果書戻し状態を現在有効と確認できない |
| E-B | 現在有効なグリッパ / 真空状態が許容範囲外にある。正常品置場の容量が事前に定義した上限に達している。現在の実行チェーンに関係する位置、容量または時間条件が許容境界を超えている |

この 9 つの判定座標は、今回の Target State Entry において発生し得る状態判定上の問題を整理するために使用する。

> **1 回の状態遷移前制御で、9 つすべての CAE-SDB Result を形成する必要はない。**

今回の Target State Entry に対して対応する判定ルールが定義され、判定に必要な根拠を取得でき、実際に Evaluation を行った場合に、その結果として該当する CAE-SDB Result を形成する。
...
の場合、必要な構造は成立しているが、画像認識結果は今回の判定根拠として現在は使用できない。

対応する D 判定により、この状態が確認された場合、

```text
C-D：
画像認識結果を、今回の Target State Entry に対する
有効な判定根拠として現在は使用できない
```
...
```text
C-B：
ワーク位置が、今回の Target State Entry に対して
事前に定義された制御境界外にある
```
...
場合、対象となる許可状態を A にマッピングし、対応する B 判定を行うことで、
