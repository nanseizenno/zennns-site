...
ここでいう目標状態入口とは、システムが次の状態や実行段階へ進む直前の判定位置を指す。
...
この二軸構造のメリットは、状態の役割と判定観点を分離できることである。

その結果、同じ状態変数に S / D / B の複数の判定を適用でき、異なる設備やシステムでも判定結果を C-S、A-D、E-B などの共通形式で表現できる。

これにより、設備やシステムごとに信号名称や実装方法が異なっても、判定結果を共通形式で整理し、制御、調停、履歴へ接続できる。
