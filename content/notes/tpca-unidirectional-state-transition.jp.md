...
summary: "状態タイプでは A → B → A のような循環を表現できる一方、実運転では A₁ → B₁ → A₂ のように新しい状態インスタンスが時間方向へ継続して生成されることを説明する。Recovery、Rollback、Return も新しい状態インスタンスへの遷移として整理し、この考え方を TPCA の状態遷移設計に採用する理由を示す。"
...
しかし、実運転で発生した状態を時間位置とともに見ると、最初の A と後の A は同じ発生状態ではない。
...
この考え方を用いると、Recovery、Rollback、Return、復帰、回流、再投入なども、過去の状態へ戻る処理としてではなく、新しい状態インスタンスへの遷移として統一して扱うことができる。
...
## 3. Recovery / Rollback / Return も新しい State Instance である

制御ソフトウェアや設備運用では、次のような名称が使用される。
...
これらの名称は、処理名、工程名、制御経路名としてそのまま使用できる。

ただし、実運転上は過去の状態インスタンスそのものへ戻るわけではない。
...
`AUTO₂` は、異常発生と Recovery を経た後に形成された新しい状態インスタンスである。
...
の場合も、`WAIT₂` は `WAIT₁` と同じ状態タイプに属する新しい状態インスタンスである。

この整理により、Recovery、Rollback、Return、復帰、回流、再投入を、
...
状態タイプの循環と状態インスタンスの時間方向を分けて扱うと、制御ソフトウェアの状態モデルを整理しやすくなる。

### 4.1 正常・異常・復帰を同じ状態遷移形式で扱える

正常処理だけでなく、異常処理、Recovery、Return、回流、再投入も、
...
そのため、復帰処理だけに別の時間方向を定義する必要がない。

### 4.2 実運転履歴を上書きせずに残せる

同じ状態タイプへ再進入した場合も、新しい状態インスタンスとして扱うことで、
...
正常経路、異常経路、復帰経路を別々の時間方向として扱わず、すべて後続する状態インスタンスとして扱うことで、状態モデル全体を一つの時間方向で記述できる。
...
ここで Target State が過去に存在した状態タイプと同じであっても、実際に進入した後の状態は新しい状態インスタンスとして扱う。
...
TPCA では、この考え方を正常実行、異常処理、復帰、回流、縮退、再投入などに共通して適用する。

したがって、TPCA における状態遷移単方向性は、

> **状態タイプの遷移関係から循環を排除することを意味しない。**

> **実運転で発生した状態インスタンスを、時間方向へ継続する一連の状態として扱う。**
...
1. 状態タイプには循環を含めることができる。

```text
A → B → A
```

2. 実運転で発生する状態インスタンスは時間方向へ継続する。

```text
A₁ → B₁ → A₂
```

3. Recovery、Rollback、Return、復帰、回流、再投入も、新しい状態インスタンスへの遷移として扱う。

この整理により、正常処理、異常処理、復帰処理を共通した状態遷移形式で表現できる。
