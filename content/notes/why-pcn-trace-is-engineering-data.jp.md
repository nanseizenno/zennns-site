...
summary: "PCN Trace と従来の設備データ、生産データ、アラーム履歴との違いを説明する。PCN Trace は、明確な目標状態入口を中心として、関連する入力状態、CAE-SDB 判定結果、制御優先度調停、複数経路制御、実行結果、時間情報 T を一つの状態遷移判定履歴として関連付ける。"
description: "明確な目標状態入口を起点として、PCN が現在状態、目標状態、複数ソース状態、C / A / E 状態マッピング、S / D / B 判定、CAE-SDB 判定結果、制御優先度調停、複数経路制御、実行結果、時間情報 T を PCN Trace として関連付ける方法を説明する。また、PLC / HMI 診断、MES / WCS 協調分析、状態遷移改善、生産 DX における活用方法を示す。"
date: 2026-07-14
lastmod: 2026-08-26
...
PCN Trace では、これらの既存データに加えて、

> **一回の目標状態入口に対する状態遷移判定**　を記録対象とする。

一回の判定では、今回の目標状態入口に対して使用した状態、形成された CAE-SDB 判定結果、制御優先度調停の結果、選択された複数経路制御、実行後に形成された状態を関連付ける。

PLC、MES、WCS、ロボット、画像認識システムなどから得られる既存の状態情報は、今回の目標状態入口に対応する一つの判定履歴として整理される。

PCN は、一回の状態遷移前判定について、現在状態、目標状態、関連する入力状態、C / A / E 状態マッピング、S / D / B 判定、CAE-SDB 判定結果、制御優先度調停、複数経路制御、実行結果、時間情報 T を関連付ける。

この状態遷移判定履歴を **PCN Trace** と呼ぶ。

PCN Trace により、次の関係を同じ目標状態入口に対応する履歴として追跡できる。
...
「なぜその時点でピックアップ段階へ入らなかったのか」を確認するには、今回の目標状態入口に対応する判定関係も必要になる。
...
PCN Trace は、これらを判定時点で同じ目標状態入口に関連付ける。
...
目標状態へ入る前に、PCN は今回の状態遷移に関係する複数ソース状態を整理し、必要な判定を行う。
...
* C / A / E は、今回の目標状態入口に関係する状態の役割を表す状態変数領域である。
* S / D / B は、それらの状態に対して適用する判定性質である。
* CAE-SDB 判定結果は、C / A / E と S / D / B の組合せによって形成される構造化判定結果である。
* 時間情報 T は、状態および判定とともに保持し、状態の前後関係、D 判定、PCN Trace に使用する。
* 制御優先度調停は、CAE-SDB 判定結果、重要な許可、制御制約、優先関係などを処理する。
* 複数経路制御は、制御優先度調停の結果に基づくエンジニアリング制御出力である。
* 実行結果は、複数経路制御の実行後に確認された状態を記録する。
...
この Trace では、次の関係を一回の目標状態入口に対応する履歴として保持する。
...
PCN Trace は、時間方向へ継続して形成される状態インスタンスと、それぞれの目標状態入口に対する判定履歴を関連付ける。
...
PCN Trace では、目標状態入口に対する主要な判定情報が生成時点で関連付けられている。
...
| 特定の複数経路制御が高頻度 | 対応する目標状態入口の制御ルール、優先関係、候補経路 |
| 特定の複数経路制御と実行結果の組合せが継続 | 制御経路と実際の実行結果との関係 |
...
例えば、ある PCN で次の CAE-SDB 判定結果が継続して発生している場合を考える。
...
これにより、対象となった目標状態、判定結果、制御経路を同じ画面または履歴情報から確認できる。
...
MES / WCS の協調停滞分析では、タスクや設備状態に加えて、目標状態入口単位の判定結果を分析対象として利用できる。

### 製造 DX

製造 DX では、設備データ、生産データ、イベントデータなどを利用して、次のような分析を行う。
...
PCN Trace は、これらの状態情報を一回の目標状態入口に対する状態遷移判定履歴として関連付ける。
...
* C / A / E は、今回の状態遷移に関係する状態の役割を表す。
* S / D / B は、各状態に対して適用する判定性質を表す。
* CAE-SDB 判定結果は、一回の状態遷移前判定で形成された構造化判定結果である。
* 時間情報 T は、状態および判定とともに保持し、状態の前後関係、動的時系列有効性、PCN Trace に使用する。
* 制御優先度調停は、判定結果と制御制約に基づいて制御上の優先関係を処理する。
* 複数経路制御は、現在の目標状態入口に対するエンジニアリング制御出力である。
* 実行結果は、複数経路制御の実行後に確認された状態を記録する。
* PCN Trace は、これらを一回の状態遷移判定履歴として関連付ける。

本稿でいう「新しいエンジニアリングデータ」は、

> **一回の目標状態入口に対する状態遷移判定そのものを、継続して記録、比較、集計、分析できるデータ対象として扱うこと**
